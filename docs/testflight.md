...
| Account deletion | The native [Account lifecycle flow](../Packages/RoomlingsCore/README.md#account-deletion) now provides confirmation, re-verification and pending-deletion recovery. Verify the deployed provider and recovery path before review. Owners with other active roommates must first use the existing web ownership settings. |
